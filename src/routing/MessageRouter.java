...
public abstract class MessageRouter {
	...
	public HashMap<String, Message> contents;
	public HashMap<String, Message> interests;
	...
	public MessageRouter(Settings s) {
		...
		if(this.getHost().getAddress() >=0 && this.getHost().getAddress() < 20 ){
			this.getHost().getLocation().setX((double)rd.nextInt(1000)+750);
			this.getHost().getLocation().setY((double)rd.nextInt(1000)+750);
		}
		
		else if(this.getHost().getAddress() < 40){
			this.getHost().getLocation().setX((double)rd.nextInt(1000)+3250);
			this.getHost().getLocation().setY((double)rd.nextInt(1000)+750);
		}
		else if(this.getHost().getAddress() < 60){
			this.getHost().getLocation().setX((double)rd.nextInt(1000)+750);
			this.getHost().getLocation().setY((double)rd.nextInt(1000)+3250);
		}
			...
			this.getHost().getLocation().setX((double)rd.nextInt(1000)+2000);
			this.getHost().getLocation().setY((double)rd.nextInt(1000)+2000);
		}
		else if(this.getHost().getAddress() == 120){
			this.getHost().getLocation().setX(1250);
			this.getHost().getLocation().setY(1250);
		}
		else if(this.getHost().getAddress() == 121){
			this.getHost().getLocation().setX(3750);
			this.getHost().getLocation().setY(1250);
		}
		else if(this.getHost().getAddress() == 123){
			this.getHost().getLocation().setX(1250);
			this.getHost().getLocation().setY(3750);
		}
		else if(this.getHost().getAddress() == 124){
			this.getHost().getLocation().setX(3750);
			this.getHost().getLocation().setY(3750);
		}
		else if(this.getHost().getAddress() == 125){
			this.getHost().getLocation().setX(2500);
			this.getHost().getLocation().setY(2500);
		}
		*/
	}
	...
	public void init(DTNHost host, List<MessageListener> mListeners) {
		...
		this.contents = new HashMap<String, Message>();
		this.interests = new HashMap<String, Message>();
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		String mid = aMessage.getId();
		String prefix = mid.substring(0,1);
		String cStr = "C";
		
		if(prefix.equals(cStr)){
			isFinalRecipient = aMessage.getRequesters().contains(this.host);
		}
	...
	protected void addToMessages(Message m, boolean newMessage) {
		...
		if(prefix.equals(cStr)){
			this.contents.put(mid, m);
		}
		else if(prefix.equals(iStr)){
			this.interests.put(cid, m);
		}
	...
	protected Message removeFromMessages(String id) {
		...
		if(prefix.equals(cStr)){
			this.contents.remove(mid);
		}
		if(prefix.equals(iStr)){
			this.interests.remove(cid);
		}
